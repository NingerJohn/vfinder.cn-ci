...
;(function($, undefined) {
	...
	$.extend(Valid.prototype, {
		...
		messages: {
			not_empty: '字段不能为空.'
			,min_length: '至少 :value 个字符.'
			,max_length: '不能超过 :value 个字符.'
			,regex: ''
			,email: '请输入email地址.'
			,url: '请输入有效的url地址.'
			,exact_length: '请输入 :value 个字符.'
			,equals: ''
			,ip: ''
			,credit_card: '请输入信用卡卡号.'
			,alpha: ''
			,alpha_numeric: ''
			,alpha_dash: ''
			,digit: '只能输入数字.'
	...
	$.fn[pluginName].defaults = {
		wrapper: 'form-group'	// the html class that wraps the field and which defines a "row" of the form
